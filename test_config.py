    ...
    try:
        ...
        for name, key in keys_to_check.items():
            if key and len(key) > 5:
                ...
                logger.warning(f"⚠️ {name} Key seems empty or too short!")

        # 3. Check optional keys
        if settings.HF_API_KEY:
            logger.info(f"ℹ️ Hugging Face Key found: {settings.HF_API_KEY[:5]}...")
        else:
            logger.info("ℹ️ Hugging Face Key is empty (Optional).")
